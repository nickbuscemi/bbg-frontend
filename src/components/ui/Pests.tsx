...
export const Pests = () => {
    ...
    useEffect(() => {
        ...
        if (insectItemsRef.current) {
            insectItemsRef.current.slice(0, 5).forEach((item, index) => {
                if (item) {
        ...
        return () => {
            ...
            if (insectItemsRef.current) {
                insectItemsRef.current.slice(0, 5).forEach((item) => {
                    if (item) {
        ...
        <section className="py-12 bg-white sm:py-16 lg:pt-20 font-helvetica-regular">
            <div className="px-4 mx-auto max-w-7xl sm:px-6 lg:px-0">
                <div ref={titleRef} className="text-center">
                    ...
                    <h1 className="mt-2 text-3xl font-bold text-black sm:text-4xl xl:text-5xl">We believe pest control begins with a deep understanding.</h1>
                    <Link to="/pest-library" className="inline-block mt-4 text-logoGreen font-semibold hover-effect">
                        View Our Entire Pest Library <span className="arrow">&rarr;</span>
                    </Link>
                </div>
                ...
                <div className="grid grid-cols-1 mt-10 text-center sm:mt-16 gap-y-8 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 xl:mt-24">
                    {insects.slice(0, 5).map((insect, index) => (
                        <div
